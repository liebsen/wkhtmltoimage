    ...
    return exec(`./iplookup ${ip}`, (err, stdout, stderr) => {
      if (err) {
        ...
        return connection.query(`INSERT INTO captures SET id = DEFAULT, ${values}, created = NOW(), updated = NOW(), enabled = 1`, function (error, results, fields) {
          ...
          // return exec(`xvfb-run --server-args="-screen 0, 1366x768x24" wkhtmltoimage --use-xserver ${url} ${filepath}`, (err, stdout, stderr) => {
          return exec(`xvfb-run --server-args="-screen 0, 1024x768x24" cutycapt --url=${url} --out=${filepath}`, (err, stdout, stderr) => {
            if (err) {
              console.log(err)
            } else {
              res.json({
